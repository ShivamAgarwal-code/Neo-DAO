...
// Pre-common code.
const precommon = require('./precommon');

// Name of the IP address cookie.
const COOKIE_NAME_IP_ADDRESS = 'ip_address';

/**
 * This function sets a flag in the Express response object
 * 	"locals" storage that lets error handlers in this app
 * 	know a response has already been sent to the client,
 * 	so they should not send a response of their own.
 *
 * @param {Object} res - A valid Express response object.
 */
function setResponseAlreadySentFlag(res) {
    let errPrefix = `(setResponseAlreadySentFlag) `;

    if (typeof res !== 'object')
        throw new Error(errPrefix + `The Express result parameter does not contain a valid object.`);

    if (!('locals' in res))
        throw new Error(errPrefix + `The Express result parameter object does not contain a field named "locals".`);

    res.locals.isResponseAlreadySent = true;
}
...
function makeHttpAsyncRequest_promise(httpMethod, url, request, headersToUse, bodyPayload)
{
    ...
    if (httpMethod == http_methods_enum.default.GET)
    {
        ...
        // Convert the payload object to a GET request suitable string
        //  of URL arguments.
...
function returnStandardErrorObj(req, res, theErrorMessage, isErrorShownToUser, httpStatusCode, objAuxArgs)
{
    ...
    // Set the flag that tells other error handlers not to send
    //  a response to the user since we have already done that.
    setResponseAlreadySentFlag(res);
 ...
 *
 * @return {String|null} - Return the current value of the
 * 	desired environment variable or NULL if there is no
 * 	environment variable with the desired name.
 */
function getEnvironmentVariableByName(envVarName) {
    ...
    if (precommon.isDevEnv() && process.env[devEnvVarName])
        // Yes.  Use it.
        tempStr = process.env[devEnvVarName];
    else
        tempStr = process.env[envVarName];
 ...
 *
 * IMPORTANT: This function will format the returned value as a DynamoDB
 * 	key, an operation that includes LOWERCASING the value!
 *
 * 	@param {Object} req - A valid Express request object.
 ...
 * This function pulls out the desired field from a POST request, with deep property
 * 	chain inspection along the way.  The field must not be empty either.
 * 	It trims the returned value but does nothing else to it.
 *
 * 	@param {Object} req - A valid Express request object.
 * 	@param {string} fieldName - The field name to extract.
 * 	@poram {number} maxLen - The maximum length the field can be.
 * 		The default maximum length is 1024 characters.
 */
function getRawFieldFromPostData(req, fieldName, maxLen = 1024) {
    let errPrefix = 'getRawFieldFromPostData';
...
/**
 * Return the app name set in the environment.
 *
 * @return {string}
 */
function getAppName() {
    let errPrefix = `(getAppName) `;

    return getEnvironmentVarOrError('APP_NAME');
}

/**
 * Given an Node.JS Express request object, check it for
 * 	a cookie with the desired name and if it exists,
 ...
 * @return {null|*}
 */
function checkSessionCookiesForValue(req, cookieName) {
    let errPrefix = `(checkSessionCookiesForIpAddress) `;

    if (!req)
        throw new Error(errPrefix + `The req parameter is not a valid object.`);

    if (isEmptyString(cookieName))
        throw new Error(errPrefix + `The cookieName parameter is empty.`);

    // Make sure a cookie with tha name exists in the cookie store.
    //
    // Parse the cookies found in the request header.
    let cookieValue = req.cookies[cookieName];

    // Got value?
    if (typeof cookieValue == 'undefined' || cookieValue.length < 1)
        // No.  Return NULL to let the caller know.
        return null;
    else
        // Yes.  Return it.
        return cookieValue;
...
/**
 * This function checks for the user's remote IP address in
 *  the session cookies.  If found, it returns it.  Otherwise,
 *  it returns NULL.
 *
 * @param {Object} req - A valid Node.JS Express request object.
 *
 * @return {null|*}
 */
function checkSessionCookiesForIpAddress(req) {
    let errPrefix = `(checkSessionCookiesForIpAddress) `;

    if (!req)
        throw new Error(errPrefix + `The req parameter is not a valid Node.JS request object.`);

    return checkSessionCookiesForValue(req,COOKIE_NAME_IP_ADDRESS);
}

/**
 * Given a Node.JS Express response object,set a
 * 	cookie with the desired name and value into it.
 *
 * @param {Object} res - A valid Node.JS Express response object.
 * @param {String} cookieName - The name of the desired cookie.
 *
 * @return {null|*}
 */
function putValueInSessionCookies(res, cookieName, cookieValue) {
    let errPrefix = `(putValueInSessionCookies) `;

    if (!res)
        throw new Error(errPrefix + `The res parameter  is not a valid Node.JS response object.`);
 ...
 *  it returns NULL.
 *
 * @param {String} ipAddress - The IP address value to store in
 *  the session cookies.
 * @param {Object} res - A valid Node.JS Express response object.
 *
 * @return {null|*}
 */
function putIpAddressInSessionCookies(res, ipAddress) {
    let errPrefix = `(checkSessionCookiesForIpAddress) `;
    ...
    if (isEmptyString(ipAddress))
        throw new Error(errPrefix + `The IP address parameter is empty.`);

    return putValueInSessionCookies(res, COOKIE_NAME_IP_ADDRESS, ipAddress);
}
...
module.exports =
    {
        ...
        checkSessionCookiesForIpAddress: checkSessionCookiesForIpAddress,
        checkSessionCookiesForValue: checkSessionCookiesForValue,
        ...
        getRawFieldFromPostData: getRawFieldFromPostData,
        ...
        putIpAddressInSessionCookies: putIpAddressInSessionCookies,
        putValueInSessionCookies: putValueInSessionCookies,
