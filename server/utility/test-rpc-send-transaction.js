// SERVER SIDE MODULE This file executes a raw transaction involving a payment (token transfer).
//  The intent is to test sending tokens from the server side.

// This code is from the Neon-js tutorial found at this URL:
//
//		https://dojo.coz.io/neo3/neon-js/docs/en/guides/basic/transfer.html
...
In Neo2, NEO and GAS are considered native assets and operate using the UTXO system.

In Neo3, the UTXO system is removed. In its place, NEO and GAS now implements the NEP-5 interface.
They are still considered native assets but operate very similarly to how NEP-5 tokens work in Neo2.

First, some setup:
 */
 
const theNodeUrl = common_routines.getEnvironmentVarOrError("URL_NEO_RPC_NODE");
...
const { CONST, rpc, sc, wallet, tx, u } = require("@cityofzion/neon-js")
...
const inputs = {
  ...
  systemFee: 0,
  networkFee: 0,
  networkMagic: theNodeNetworkMagic, //CONST.MAGIC_NUMBER.TestNet,
...
The token exists. This can be done by performing a invokeFunction call.
The amount of token exists on fromAccount.
The amount of GAS for fees exists on fromAccount.
...
/**
Network fees pay for the processing and storage of the transaction in the network. There is a cost incurred per byte of the transaction (without the signatures) and also the cost of running the verification of signatures.
 */
...
/**
 * First, we check that the token exists. We perform an invokeFunction RPC call which calls the name method of the contract. The VM should exit successfully with HALT and give us the token name if it exists.
 */
...
/**
 * SystemFees pay for the processing of the script carried in the transaction. We can easily get this number by using invokeScript with the appropriate signers.
 */
...
/**
 * We will also need to check that the inital address has sufficient funds for the transfer. We look for both funds of the token we intend to transfer and GAS required to pay for the transaction. For this, we rely on the NEP5Tracker plugin. Hopefully, the node we select has the plugin installed.
 */
...
/**
 * You should be able to see the transaction hash printed in the console log. After waiting for the network to process the transaction, you can check on your new account balance.
 */
