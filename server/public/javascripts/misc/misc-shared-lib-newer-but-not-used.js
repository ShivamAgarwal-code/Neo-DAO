 ...
 * 	otherwise, if bErrorIfInvalid is TRUE then an error will be thrown using the given
 * 	error prompt.  If bErrorIfInvalid is FALSE, NULL will be returned.
 */
function checkFieldForNumber(fieldValue, errPrompt, bErrorIfInvalid=true)
{
	...
	try
	{
		// If it is a string, try to parse it as a floating point number.
		let theNumber = parseFloat(fieldValue);
		
		// Check for NaN
		if (isNaN(theNumber)) {
			if (bErrorIfInvalid)
				throw new Error(errPrefix + 'The field value is not a number (NAN).');
			else
				return null;
		}
